...
def readrawsurface(path: str, namepatch: str="forces", time_name: str="0", name: str="forces", region:str|None=None):
    """read the raw data contained in the file about the surface
    TODO rewrite
    Args:
        path: str\n
        namepatch: str\n
    ...
    Returns:
        array: array of surface field; size of the array is the size of the
        field

    A way you might use me is:\n
        force = readforce(path='path_of_OpenFoam_case', namepatch='forces',
                          time_name='0', name='forces', region='fluidZone')
    ...
    if os.path.exists(path+namepatch):
        ...
        path_namepatch = os.path.join(path,'postProcessing',region,namepatch,'surface')
    if time_name == "latestTime":
        time_name = _find_latesttime(path_namepatch)
    elif time_name == "mergeTime":
        time_list = []
    ...
    with open(os.path.join(path_namepatch, time_name, name + ".raw"), "rb") as f:
        content = f.read()
    data = content.split(b"\n")
    j = 0
    header = True
